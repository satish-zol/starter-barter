...
class JobsController < ApplicationController
  ...
  def index
    ...
    @jobs = Job.joins("LEFT JOIN users ON users.id = jobs.user_id").where("jobs.job_status=? AND users.iam LIKE ?", "Open", "%#{current_user.iam}%").order("created_at DESC").paginate(:page => params[:page], :per_page => 10)
